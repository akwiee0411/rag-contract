...
def fetch_qa_candidates(message: str, txt_path: str, rag_available: bool = False):
    ...
    html = (f"<div style='padding:6px;'>"
            ...
            f"<p style='font-size:0.82em;color:white;margin:4px 0 6px;'>"
...
border-radius:6px;background:##003060;'>
  ...
  <div style='background:gray;padding:6px 8px;border-radius:4px;
  font-size:0.88em;margin-bottom:4px;'>
    <strong>❓</strong> {q[:120]}{"..." if len(q)>120 else ""}
  </div>
  <div style='background:gray;padding:6px 8px;border-radius:4px;
  font-size:0.85em;color:white;max-height:60px;overflow:hidden;'>
...
def auto_qa_response(message: str, txt_path: str, history: list,
                     rag_engine, use_complete_sections: bool,
                     selected_model: str, lang: str,
                     rag_available: bool = False):
    """
    全自動問答主入口（取代舊的「展示快取候選讓使用者選擇」流程）。

    流程：
      1. 在 qa_cache.json 中搜尋與問題最相似的快取問答（top_k=1）。
      2. 最高相似度 >= SIMILARITY_THRESHOLD（預設 0.8）：
           → 直接以該快取答案回覆使用者，不呼叫 LLM。
           → 回覆訊息前綴「💾 (快取答案，相似度 X.XXX)」，讓使用者知道來源。
      3. 最高相似度 < 門檻 或 無任何快取：
           → 直接呼叫 LLM（rag_chat_response），生成新答案。

    Parameters
    ----------
    message              : 使用者輸入的問題字串
    txt_path             : 合約 .txt 路徑（用於定位 qa_cache.json）
    history              : 目前 chatbot 的訊息歷史（list of dict）
    rag_engine           : HierarchicalQueryEngine（快取套用時重新 retrieve 條款用）
    use_complete_sections: 是否顯示完整章節
    selected_model       : LLM 模型名稱（LLM fallback 時使用）
    lang                 : 輸出語言（LLM fallback 時使用）
    rag_available        : True = 可使用向量相似度；False = 字元重疊率

    Returns
    -------
    tuple (4 values)：
      history        : 更新後的 chatbot 訊息歷史
      retrieval_html : 右側「🔍 檢索原始條款」面板的 HTML
      side_panel_upd : gr.update，控制側邊欄顯示狀態
      pending_qa     : 本次問答資料（供儲存 / 回報使用；截斷時為 None）
    """
    # ── 文件未載入或知識庫未就緒：直接走 LLM ─────────────────
    if not txt_path or rag_engine is None:
        return rag_chat_response(
            message, history, rag_engine,
            selected_model, lang, use_complete_sections, txt_path
        )

    # ── 查詢快取（只取 top-1，門檻判斷只需要最高分）────────────
    candidates = retrieve_similar_qa(
        message, txt_path, top_k=1, rag_available=rag_available
    )

    if candidates and candidates[0][1] >= SIMILARITY_THRESHOLD:
        # ── 相似度達標：自動套用快取答案 ─────────────────────
        entry, score = candidates[0]
        result = apply_cached_answer(
            candidates,          # [(entry, score)]
            "#1",                # 選第 1 筆（0-based index 0）
            message,
            list(history),       # 傳副本，避免 in-place 污染
            use_complete_sections,
            rag_engine,
        )
        new_history, retrieval_html, side_panel_upd, pending_qa, _ = result
        # 顯示 cand_group，讓「呼叫AI 生成」按鈕可見（供使用者覆蓋）
        return new_history, retrieval_html, side_panel_upd, pending_qa, gr.update(visible=True)

    else:
        # ── 相似度不足：呼叫 LLM 生成新答案 ─────────────────
        result = rag_chat_response(
            message, history, rag_engine,
            selected_model, lang, use_complete_sections, txt_path
        )
        # rag_chat_response 已回傳 5 個值，第 5 個是 cand_group update
        return result
...
def apply_cached_answer(candidates: list, selected_label: str, question: str,
    ...
    """
    使用者從候選面板選了某筆快取答案後呼叫。
    答案文字來自快取（不呼叫 LLM），但右側檢索條款會針對當前問題重新 retrieve。

    【方案 B：快取答案 + 重新 Retrieve】
    答案文字：直接使用快取記錄，速度快、節省 GPU 資源。
    相關條款：用當前問題重新呼叫 rag_engine.query_with_complete_sections，
              確保右側面板顯示的是針對「現在這個問題」檢索到的條款，
              而不是快取儲存當時的舊條款。
    若 rag_engine 為 None（知識庫未就緒），退回使用快取的舊條款，不報錯。

    【selected_label 解析邏輯】
    Radio 的 label 格式為「#N 相似度 ...」，取開頭的「#N」解析出 0-based 索引。
    解析失敗時預設使用第一筆（index=0），以 max/min 確保索引在合法範圍內。
    ...
    question             : 使用者目前輸入的問題
    history              : 當前 chatbot 的訊息歷史列表
    use_complete_sections: 是否渲染完整章節面板
    rag_engine           : HierarchicalQueryEngine 物件，用於重新 retrieve 條款
    ...
    current_q     = question or orig_question

    # ── 重新 Retrieve 相關條款（方案 B 核心）────────────────────
    # 用當前問題重新查詢 RAG，取得針對這個問題的最新條款
    # 答案文字仍使用快取，只有右側面板的條款是重新查詢的結果
    rel_clauses   = entry.get("related_clauses", [])   # 預設：快取舊條款（作為 fallback）
    complete_secs = entry.get("complete_sections", []) # 預設：快取舊章節（作為 fallback）

    retrieve_note = "（條款來自快取）"  # 預設備註，重新 retrieve 成功後會更新

    if rag_engine is not None:
        try:
            rag_result    = rag_engine.query_with_complete_sections(
                current_q, include_complete_sections=use_complete_sections
            )
            # 只取 retrieve 結果，不用 rag_result["answer"]（答案還是用快取的）
            rel_clauses   = rag_result.get("related_clauses", rel_clauses)
            complete_secs = rag_result.get("complete_sections", complete_secs)
            retrieve_note = "（條款已針對當前問題重新檢索）"
        except Exception as e:
            # retrieve 失敗時靜默 fallback，不影響快取答案顯示
            print(f"⚠️ apply_cached_answer 重新 retrieve 失敗，使用快取條款：{e}")
            retrieve_note = "（條款 retrieve 失敗，顯示快取舊條款）"

    # 快取標識 badge，顯示相似度、原問題、以及 retrieve 狀態
    cache_badge = (
        ...
        f"{'...' if len(orig_question)>60 else ''}」"
        f"<br><small style='color:#555;'>{retrieve_note}</small></div>"
    )
    ...
    full_info_html = (
        ...
        f"<div style='background:grey;padding:12px;margin-bottom:15px;border-radius:4px;'>"
        f"<strong>📊 檢索結果：</strong> {len(rel_clauses)} 個條款"
    ...
    # pending_qa 使用重新 retrieve 到的條款，確保儲存的內容是最新的
    pending_qa = {
        "question":          current_q,
        "answer":            response,
...
border-radius:8px;background:#fff;'>
  ...
  <div style='background:gray;padding:10px;border-radius:4px;margin-bottom:8px;'>
    <strong>❓ 問：</strong>{q}
  </div>
  <div style='background:gray;padding:10px;border-radius:4px;max-height:110px;overflow-y:auto;'>
...
def delete_folder(folder_name: str) -> tuple:
    """
    刪除指定的合約資料夾（包含所有圖片、txt、快取檔案）。

    安全機制：
      - folder_name 為空時拒絕刪除
      - 資料夾不存在時回傳提示，不拋出例外
      - 只允許刪除 TEXT_RESULT_DIR 下的直接子資料夾，
        防止路徑穿越攻擊（如 folder_name = "../../system"）

    Parameters
    ----------
    folder_name : 要刪除的資料夾名稱（非完整路徑）

    Returns
    -------
    tuple (status_msg, updated_choices, dropdown_update)
      status_msg       : 操作結果訊息字串
      updated_choices  : 刪除後最新的資料夾名稱列表
      dropdown_update  : gr.update，重設 Dropdown 的 choices 和 value
    """
    import shutil

    if not folder_name:
        choices = get_history_folders()
        return "⚠️ 請先選擇要刪除的資料夾", choices, gr.update(choices=choices, value=None)

    root        = Path(TEXT_RESULT_DIR)
    target      = root / folder_name

    # 安全檢查：確保 target 是 root 的直接子目錄，不接受 ".." 等路徑
    try:
        target.resolve().relative_to(root.resolve())
    except ValueError:
        choices = get_history_folders()
        return "❌ 非法路徑，拒絕刪除", choices, gr.update(choices=choices, value=None)

    if not target.exists():
        choices = get_history_folders()
        return f"⚠️ 資料夾「{folder_name}」不存在", choices, gr.update(choices=choices, value=None)

    try:
        shutil.rmtree(target)
        choices    = get_history_folders()
        status_msg = f"✅ 已刪除資料夾「{folder_name}」（包含所有檔案）"
    except Exception as e:
        choices    = get_history_folders()
        status_msg = f"❌ 刪除失敗：{e}"

    return status_msg, choices, gr.update(choices=choices, value=None)

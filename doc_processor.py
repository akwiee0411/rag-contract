...
本模組負責「原始文件 → RAG 可查詢索引」的完整處理流程：
  ...
  D. 臨時模式（不存檔、每次刷新）
     - 文字提取結果存入系統 tempdir，不寫 TEXT_RESULT_DIR（pdf_process_temp）
     - RAG 索引完全在記憶體中建立，不讀也不寫任何快取（build_temp_rag_index）
...
def pdf_process_temp(pdf_path: str, ocr_model, ocr_processor,
                     progress=None) -> tuple[str, str, str]:
    """
    臨時模式的 PDF 文字提取：結果存在系統 tempdir，不寫入 TEXT_RESULT_DIR。
    每次呼叫都建立全新的 tempdir，不留下任何持久化檔案。

    【與 pdf_process_and_save 的差異】
    - 輸出目錄：系統 tempdir（每次新建）而非 TEXT_RESULT_DIR
    - 無快取：每次都重新處理，不讀也不寫任何快取
    - 回傳值多一個 tmpdir 路徑供呼叫端清理

    Parameters
    ----------
    pdf_path      : 要處理的 PDF 路徑
    ocr_model     : OCRFlux-3B 模型物件（純文字 PDF 傳 None 也可）
    ocr_processor : 對應的 AutoProcessor
    progress      : Gradio progress 物件（可選）
    ...
    """
    import tempfile
    tmpdir = tempfile.mkdtemp(prefix="sinbon_temp_")

    if progress:
        progress(0.1, desc="初始化臨時工作區...")
    ...
    pagelist = []
    for i, p in enumerate(pages):
        p_path = os.path.join(tmpdir, f"page{i}.jpg")
        pagelist.append(p_path)
        p.save(p_path, "JPEG")
    ...
    if use_plumber:
        method = "PDFPlumber"
        if progress:
            progress(0.5, desc="PDFPlumber 提取文字...")
        with pdfplumber.open(pdf_path) as pdf:
        ...
        method = "OCR"
        if progress:
            progress(0.5, desc="OCR 辨識中...")
        for i, img_path in enumerate(pagelist):
            if progress:
                progress(0.5 + 0.4 * i / len(pagelist),
                         desc=f"OCR 第 {i+1}/{len(pagelist)} 頁...")
            try:
                res = ocr_page(img_path, ocr_model, ocr_processor, 15000)
                try:
                    j = json.loads(res)
                    full_text += j.get("natural_text", str(j))
                except Exception:
    ...
    txt_path = os.path.join(tmpdir, f"{fname}.txt")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write(full_text)

    if progress:
        progress(1.0, desc="完成")
    return f"完成 ({method})", txt_path, tmpdir


def build_temp_rag_index(txt_path: str, rag_available: bool, progress=None):
    """
    臨時模式的 RAG 索引建立：完全在記憶體中建立，不 persist 到磁碟。
    不讀取既有快取，確保每次都是乾淨的新 index。

    【與 build_or_load_index 的差異】
    - 不讀快取：即使磁碟上有快取也不使用
    - 不寫快取：建立完成後不 persist 到磁碟
    - StorageContext.from_defaults() 不帶 persist_dir → 純記憶體

    Parameters
    ----------
    txt_path      : 合約 .txt 檔案路徑（通常在 tempdir）
    rag_available : 系統是否有可用的 LlamaIndex + Embedding 模型
    progress      : Gradio progress 物件（可選）
    ...
    -------
    tuple[HierarchicalQueryEngine | None, str]：
      同 build_or_load_index 的回傳值，但不做任何磁碟寫入
    """
    ...
    parser = HierarchicalContractNodeParser()
    nodes  = parser.get_nodes_from_documents([doc])
    print(f"📊 [臨時模式] 解析完成，共 {len(nodes)} 個節點")

    if not nodes:
        return None, "❌ 文件解析失敗，未能提取任何節點"

    if progress:
        progress(0.45, desc=f"⚙️ 計算 Embedding（{len(nodes)} 個節點）...")

    try:
        # StorageContext.from_defaults() 不帶 persist_dir → 純記憶體，不寫磁碟
        storage_context = StorageContext.from_defaults()
        index = VectorStoreIndex(nodes, storage_context=storage_context,
                                 show_progress=False)
    except BaseException as e:
        import traceback
        print("[ERROR] 建立臨時向量索引失敗：")
        print(traceback.format_exc())
        return None, f"❌ 建立向量索引失敗：{type(e).__name__}: {e}"
    ...
    return engine, f"✅ 臨時索引建立完成（{len(nodes)} 個節點），不存入快取"

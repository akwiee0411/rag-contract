...
class HierarchicalContractNodeParser(NodeParser):
    ...
    def debug_potential_clauses(self, text: str):
        ...
        for i, clause in enumerate(main_clauses):
            ...
            # 關鍵判斷：基於動態確定的主層級判斷是否為主條款
            is_main_clause = (clause_level == primary_level)
            
            print(f"處理條款: {clause['number']}, 層級: {clause_level}, 是否主條款: {is_main_clause} (主層級={primary_level})")
            ...
            if is_main_clause:
                ...
                for j in range(i + 1, len(main_clauses)):
                    next_clause = main_clauses[j]
                    next_level = next_clause['pattern_idx']
                    
                    if next_level == primary_level:  # 遇到下一個主條款就停止
                        break
                    
                    # 將子條款內容也加入當前主條款的完整內容中
                    next_start = next_clause['line_idx']
                    if j + 1 < len(main_clauses):
                        next_end = main_clauses[j + 1]['line_idx']
                    else:
            ...
            if clause_text:
                ...
                single_section = {
                    ...
                    'type': 'main_clause' if is_main_clause else 'sub_clause',
                    'level': clause_level,
                    'clause_id': f"Clause_{len(sections)+1}",
                    'parent_clause_id': f"Section_{section_counter-1}" if not is_main_clause and current_main_clause else '',
                    'parent_clause_title': current_main_clause['number'] if not is_main_clause and current_main_clause else '',
                    'section_number': section_counter if is_main_clause else section_counter - 1,
                    'is_main_clause': is_main_clause,
                    'has_sub_clauses': False,
                    'hierarchy_path': self._build_hierarchy_path(clause, current_main_clause if not is_main_clause else None)
                }
...
class ContractJSONLReader:
    """
    專用於合約 JSONL 格式的讀取器
    
    這個讀取器專門處理合約相關的JSONL文件，能夠：
    1. 批量讀取JSONL格式的合約文件
    2. 智能提取合約文本內容
    3. 生成適當的元數據
    4. 處理各種可能的數據格式
    ...
    def __init__(self, jsonl_dir: str = "./jsonl"):
        ...
        Args:
            jsonl_dir: JSONL文件目錄路徑
        """
        self.jsonl_dir = jsonl_dir
        if not os.path.exists(jsonl_dir):
            os.makedirs(jsonl_dir)
            print(f"已創建目錄: {jsonl_dir}")
    ...
    def load_data(self) -> List[Document]:
        ...
        if not jsonl_files:
            print(f"在 {self.jsonl_dir} 目錄下沒有找到 .jsonl 檔案")
            return documents
        
        # 逐個處理JSONL文件
        for jsonl_file in jsonl_files:
            file_path = os.path.join(self.jsonl_dir, jsonl_file)
            print(f"正在讀取: {jsonl_file}")
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    for line_num, line in enumerate(f, 1):
                        ...
                        try:
                            ...
                            text_content = self._extract_contract_text(data)
                            
                            if text_content:
                                contract_id = self._extract_contract_id(data, jsonl_file)
                                ...
                                doc = Document(
                                    ...
                                    metadata={
                                        ...
                                        "orig_path": data.get("orig_path", "N/A"),
                                        "num_pages": data.get("num_pages", 0),
                                        "document_type": "contract",
                                        
                                        # 其他元數據
                                        **self._extract_additional_metadata(data)
                                    }
                                )
                                documents.append(doc)
                                print(f"  成功載入合約: {contract_id}")
                            
                        except json.JSONDecodeError as e:
                            print(f"解析 JSON 失敗 ({jsonl_file}, 第{line_num}行): {e}")
                            continue
            
            except Exception as e:
                print(f"讀取檔案失敗 {jsonl_file}: {e}")
                continue
        
        print(f"總共載入 {len(documents)} 個合約文件")
        return documents
    
    def _extract_contract_text(self, data: Dict[str, Any]) -> str:
        """
        從 JSON 數據中提取合約文本
        ...
        # 備選字段按優先級順序
        text_fields = ['document_text','text', 'content', 'contract_text', 'document']
    ...
    def _extract_contract_id(self, data: Dict[str, Any], filename: str) -> str:
        """
        提取合約 ID
        ...
        if 'orig_path' in data:
            ...
            contract_id = os.path.splitext(os.path.basename(orig_path))[0]
            return contract_id
    ...
    def _extract_additional_metadata(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 排除的字段列表
        exclude_fields = {
            "natural_text",'document_text', 'text', 'content', 'contract_text', 'document', 
            'orig_path', 'num_pages', 'page_texts', 'fallback_pages'
        }

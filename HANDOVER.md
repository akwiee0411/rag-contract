...
**位置**：`doc_processor.py` `_pdf_word_count` 函式，第 428 行呼叫

```python
use_plumber = _pdf_word_count(pdf_path) > 50
```

判斷邏輯：讀 PDF 第一頁的文字字元數，超過 50 用 PDFPlumber，否則用 OCR。
...
|------|---------|------|
| 現在（預設） | 50 | 合適大多數合約 |
| 常遇到「有浮水印文字的掃描檔被誤判為原生 PDF」 | 提高到 100~200 | 浮水印可能讓第一頁有少量文字但實際是掃描 |
| 合約封面頁只有 logo 圖片加少量文字 | 降低到 20~30 | 避免封面頁文字太少被誤判為掃描 |

### OCR max_new_tokens

**位置**：`doc_processor.py` 第 454 行
...
| 數值 | 適用情境 | 缺點 |
|------|---------|------|
| 4096（函式預設值） | 一般文字段落頁 | 可能截斷有大量文字的頁面 |
| **15000（目前值）** | 表格密集的合約頁面 | 每頁推理時間增加，整份文件 OCR 可能要數十分鐘 |
| 8192 | 折衷方案 | 適合大多數合約，兼顧速度和完整性 |

### 預覽圖 DPI

**位置**：`doc_processor.py` 第 145 行

```python
return convert_from_path(pdf_path, dpi=120, first_page=1, last_page=pages)
```

- `dpi=120`：預覽用，清晰度夠，記憶體較省
- 調高到 `dpi=200`：更清晰，適合使用者需要看清合約細節時
- **不建議設為 300**：這是 OCR 用的解析度，用在 UI 預覽會讓記憶體用量大增
...
**位置**：`parsetool.py` 第 819 行

```python
self.base_retriever = index.as_retriever(similarity_top_k=3)
```

這個值決定「每次問答前，先從向量索引抓幾個最相似的節點」。

| 數值 | 效果 | 代價 |
|------|------|------|
| 2 | 快速，context 短 | 可能遺漏相關條款，尤其複雜問題 |
| **3（目前）** | 平衡 | — |
...
**索引順序即優先順序**：索引 0（`第X條`）是最高優先級的條款格式。
...
OCR 進行中強制關閉程式，`text_result` 下的合約資料夾可能只有 `page0.jpg` 而沒有 `.txt` 檔案。下次啟動後這個資料夾仍會出現在歷史清單，但點「載入此專案」時會因為找不到 `.txt` 而報錯。需手動刪除此資料夾。

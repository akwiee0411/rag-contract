...
- **文字提取**：呼叫 `doc_processor.pdf_process_temp`，結果存入 `tmpdir`（`tempfile.mkdtemp(prefix="sinbon_temp_upload_")` 建立），**不寫** `TEXT_RESULT_DIR`
- **RAG 索引**：呼叫 `doc_processor.build_temp_rag_index`，索引完全在記憶體中建立，**不讀也不寫**任何快取
- **tmpdir 清除有兩個觸發點**：
  1. 使用者點「重設」→ `_temp_reset` 呼叫 `shutil.rmtree(tmpdir)`（安全檢查：路徑中必須含 `"sinbon_temp_"` 字串才刪除）
  2. 使用者**再次上傳新檔**→ `_temp_process_upload` 在建立新 `tmpdir` 前，先刪除 `state_data["tmpdir"]` 指向的舊目錄

**⚠️ 已知缺口：PDF 切割的公共暫存殘留**

`_temp_split_document` 在切割時呼叫的是 `doc_processor.split_pdf`，而該函式固定寫出到公共暫存目錄 `TEMP_DIR`（即 `TEMP_SPLIT_DIR`），再由 `_temp_split_document` 用 `shutil.copy2` 複製一份到 `tmpdir`。**公共 `TEMP_DIR` 裡的那份切割 PDF 不會被 Tab 7 的任何清除邏輯刪除**，只有當 Tab 2 的使用者下次上傳新檔（觸發 `cleanup_temp_dir()`）或程式重啟時，才會被清空。若 Tab 7 是為了「不留痕跡」的需求設計，這個行為需要留意。
...
| Tab 7 tmpdir 建立前綴 | `_temp_process_upload` 第 1838 行 | `"sinbon_temp_upload_"` | `mkdtemp` 建立目錄時使用的前綴，決定暫存目錄的名稱 |
| Tab 7 tmpdir 安全刪除字串 | `_temp_reset` 第 2021 行 | `"sinbon_temp_"` | `_temp_reset` 刪除前的安全檢查字串（比建立前綴短，刻意設計成能同時匹配新舊前綴）。⚠️ 若修改建立前綴，確保安全字串仍是新前綴的子字串 |
...
`tmpdir` 在 `_temp_process_upload` 時用 `tempfile.mkdtemp(prefix="sinbon_temp_upload_")` 建立，所有文字提取結果和切割暫存 PDF 都寫進這個目錄，而非 `TEXT_RESULT_DIR` 或 `TEMP_SPLIT_DIR`。

**tmpdir 的兩個清除觸發點：**
1. **點「重設」**：`_temp_reset` 檢查路徑含 `"sinbon_temp_"` 後刪除
2. **再次上傳新檔**：`_temp_process_upload` 在建立新 `tmpdir` 前先刪除舊的

注意：安全刪除字串 `"sinbon_temp_"` 與建立前綴 `"sinbon_temp_upload_"` 是**不同字串**。前者是後者的子字串，刻意設計成這樣，確保安全檢查能匹配到所有 Tab 7 產生的暫存目錄。若要修改前綴，兩個字串都需要一起調整。
...
### H. Tab 7 暫存目錄的清除行為與殘留風險

**正常清除觸發點有兩個：**
1. 使用者**主動點「重設」**→ `_temp_reset` 刪除 `tmpdir`
2. 使用者**在 Tab 7 再次上傳新檔**→ `_temp_process_upload` 先刪舊 `tmpdir` 再建新的

**以下情境會導致 tmpdir 殘留**（位於系統 `%TEMP%` 目錄，前綴 `sinbon_temp_upload_`，大型合約可能達數十 MB）：
- 使用者直接關閉瀏覽器，未點重設
- 程式崩潰
- 使用者結束 Tab 7 後沒有繼續操作，直接使用其他 Tab

需手動到系統暫存目錄刪除，或等下次程式重啟。

**⚠️ 額外殘留：公共 TEMP_DIR 的切割 PDF**

`_temp_split_document` 呼叫 `split_pdf`，後者固定寫出到 `TEMP_DIR`（公共 `TEMP_SPLIT_DIR`），再 `copy2` 到 `tmpdir`。公共那份**不受 Tab 7 任何清除邏輯管理**，只有下列情況才會被刪除：
- Tab 2 使用者上傳新檔（觸發 `cleanup_temp_dir()`）
- 程式重啟（`TEMP_DIR` 在啟動時整個刪除重建）

若 Tab 7 是為了不留痕跡而設計，此行為是一個已知缺口。修復方式：在 `_temp_split_document` 的 `copy2` 之後，加一行 `out_path.unlink(missing_ok=True)` 刪除 `TEMP_DIR` 裡的原始切割檔。

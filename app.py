...
負責：
  1. 模型呼叫（透過遠端 API 呼叫 5090 的 OCR / Embedding / Ollama LLM）
  2. Gradio 介面定義（八個 Tab、所有 UI 元件）
  3. 所有 UI 事件的邏輯函式（上傳、切割、OCR、RAG 問答、智能搜尋）
  4. Gradio State 管理（file_state 貫穿整個處理流程）

【雙機部署架構】
  5090 機器：跑 model_server.py（OCR + Embedding API）+ Ollama（LLM）
  Client 機器：跑本程式（Gradio UI + 業務邏輯），透過 HTTP API 呼叫 5090 的模型

【需要在 Client 設定的環境變數】
  MODEL_SERVER_URL : 5090 的 model_server.py 位址（預設 http://localhost:8000）
  OLLAMA_HOST      : 5090 的 Ollama 位址（預設 http://localhost:11434）
  TEXT_RESULT_DIR  : 共用的合約儲存目錄（兩台機器須能共同讀寫，可用網路磁碟）
"""
...
#
#   2. Model Server（OCR + Embedding）：
#      python model_server.py
#
# 【Client 機器啟動前設定環境變數】
#   Linux/Mac:
#     export MODEL_SERVER_URL=http://192.168.x.x:8000
#     export OLLAMA_HOST=http://192.168.x.x:11434
#     export TEXT_RESULT_DIR=/mnt/rag_contract/text_result
#   Windows:
#     set MODEL_SERVER_URL=http://192.168.x.x:8000
#     set OLLAMA_HOST=http://192.168.x.x:11434
#     set TEXT_RESULT_DIR=\\192.168.x.x\rag_contract\text_result

# 5090 Model Server 的 URL（OCR + Embedding API）
...
# 所有合約結果的根儲存目錄（兩台機器必須能讀寫同一個目錄）
TEXT_RESULT_DIR = os.environ.get("TEXT_RESULT_DIR",
                                 r"C:\Users\user\Documents\rag_contract\text_result")

# ── 本地模組 ──────────────────────────────────────────────────
from cache_manager import (
    list_cached_collections, delete_cache_for_txt, delete_all_cache,
    delete_folder_qa_cache, delete_all_qa_cache, delete_qa_entry_by_id,
)
from doc_processor import (
    ...
    pdf_process_temp, build_temp_rag_index,
)
from ui_helpers import (
    get_history_folders, preview_history_folder,
    get_doc_images, toggle_doc_images,
    fetch_qa_candidates, apply_cached_answer, auto_qa_response,
    ...
    update_smart_doc_switcher, delete_folder,
)
...
def get_model_list():
    ...
    try:
        ...
        def parse_qwen(m):
            ...
            r = re.match(r"qwen(\d+(?:.\d+)?):(\d+(?:.\d+)?)b", m.lower())
            return (float(r.group(1)), float(r.group(2))) if r else None
        models = [m["model"] for m in _ollama_client.list()["models"]]
            
        qwen = [(m, *parse_qwen(m)) for m in models if parse_qwen(m)]
        others = [m for m in models if not parse_qwen(m)]
        
        qwen_sorted = [m for m,_ , _ in sorted(qwen, key=lambda x: (-x[1], -x[2]))]
        result = qwen_sorted + others
        ml=[]
        for m in result:
            ml.append(m)
...
# ── 模型狀態（透過 API 查詢遠端狀態，本地不載入任何模型）──────
# Client 機器不持有任何模型物件，所有模型都在 5090 的 model_server.py 中管理。
# RAG_AVAILABLE 僅表示「是否能連線到遠端 Embedding API」。
RAG_AVAILABLE = False


def _model_server_get(path: str) -> dict:
    """向 5090 Model Server 發送 GET 請求，失敗時回傳 {"error": "..."}。"""
    try:
        resp = httpx.get(f"{MODEL_SERVER_URL}{path}", timeout=10.0)
...
def _model_server_post(path: str) -> dict:
    """向 5090 Model Server 發送 POST 請求，失敗時回傳 {"error": "..."}。"""
    try:
        resp = httpx.post(f"{MODEL_SERVER_URL}{path}", timeout=30.0)
        resp.raise_for_status()
        return resp.json()
    except Exception as e:
        return {"error": str(e)}
...
def load_ocr_model():
    """
    通知 5090 Model Server 載入 OCR 模型（POST /ocr/load）。
    Client 端不載入任何模型，僅呼叫遠端 API。

    Returns
    -------
    bool : API 呼叫成功為 True，失敗為 False
    """
    print("通知 5090 載入 OCR 模型...")
    result = _model_server_post("/ocr/load")
    if "error" in result:
        print(f"⚠️ OCR 載入失敗：{result['error']}")
        return False
    print(f"✅ OCR 模型載入完成（5090 狀態：{result}）")
    return True


def load_embedding_model():
    """
    初始化遠端 Embedding（連線到 5090 的 /embed API）與 Ollama LLM 設定。
    修改全域變數 RAG_AVAILABLE。

    【雙機部署說明】
    本地不載入任何 Embedding 模型；Settings.embed_model 改為
    RemoteEmbedding，所有向量計算都透過 HTTP 送到 5090 執行。

    Returns
    -------
    bool : 初始化成功為 True，失敗為 False
    """
    ...
    try:
        ...
        # 確認 Model Server 可連線
        health = _model_server_get("/health")
        if "error" in health:
            print(f"⚠️ 無法連線到 Model Server ({MODEL_SERVER_URL})：{health['error']}")
            print("   請確認 5090 的 model_server.py 已啟動")
            RAG_AVAILABLE = False
            return False

        print(f"✅ Model Server 連線成功：{health}")

        # 通知 Ollama 釋放 VRAM（讓模型等需要時再載入）
        try:
            _ollama_client.generate(model="qwen3.5:4b", prompt="", keep_alive=0)
            print("✅ Ollama 已準備就緒")
        except Exception:
            pass

        # LLM：指向遠端 Ollama
        Settings.llm = Ollama(model="qwen3.5:4b", request_timeout=1200.0,
                              base_url=OLLAMA_HOST)

        # Embedding：指向遠端 Model Server 的 /embed API
        Settings.embed_model = RemoteEmbedding(base_url=MODEL_SERVER_URL)

        RAG_AVAILABLE = True
        print("✅ 遠端 RAG / Embedding 初始化完成")
        return True
...
# ── 程式啟動：連線遠端服務（不載入本地模型）──────────────────
# Client 端不持有任何 GPU 模型，只建立與 5090 的連線設定。
# Embedding 使用 RemoteEmbedding 透過 HTTP 計算，OCR 也走 API。
...
def _get_ollama_running_models_html():
    """
    查詢 Ollama 目前佔用記憶體（VRAM/RAM）的模型（在 5090 上執行）。
    透過 ollama.Client 連線到遠端 Ollama，回傳 HTML 表格字串。
    """
    try:
        result = _ollama_client.ps()
        models = result.get("models", [])
        if not models:
            return "<p style='color:gray;padding:10px;'>⚠️ 目前沒有模型佔用記憶體</p>"
        rows = ""
        for m in models:
            name    = m.get("model", "—")
            ...
            rows += (
                ...
                f"<td style='padding:6px 12px;border-bottom:1px solid #ddd;text-align:right;'>{size_gb:.2f} GB</td>"
                f"<td style='padding:6px 12px;border-bottom:1px solid #ddd;text-align:right;'>{vram_gb:.2f} GB</td>"
                f"<td style='padding:6px 12px;border-bottom:1px solid #ddd;'>{str(expires)[:19]}</td>"
                f"</tr>"
            )
        html = (
            "<table style='width:100%;border-collapse:collapse;font-size:0.95em;'>"
            "<thead><tr style='background:grey;'>"
            "<th style='padding:6px 12px;text-align:left;'>模型名稱</th>"
            "<th style='padding:6px 12px;text-align:right;'>總大小</th>"
            "<th style='padding:6px 12px;text-align:right;'>VRAM 佔用</th>"
            "<th style='padding:6px 12px;text-align:left;'>到期時間</th>"
            "</tr></thead>"
            f"<tbody>{rows}</tbody></table>"
        )
        return html
    except Exception as e:
        return f"<p style='color:red;padding:10px;'>❌ 無法取得 Ollama 狀態：{e}</p>"


def _get_ocr_status():
    """查詢 5090 Model Server 上 OCR 模型的載入狀態。"""
    result = _model_server_get("/health")
    if "error" in result:
        return f"❌ 無法連線 Model Server：{result['error']}"
    ocr = result.get("ocr", "unknown")
    return f"✅ 已載入（5090 GPU）" if ocr == "loaded" else "❌ 未載入"


def _get_emb_status():
    """查詢 5090 Model Server 上 Embedding 模型的載入狀態。"""
    result = _model_server_get("/health")
    if "error" in result:
        return f"❌ 無法連線 Model Server：{result['error']}"
    emb = result.get("embed", "unknown")
    return (f"✅ 已載入（5090 CPU，遠端 API）"
            if emb == "loaded" else "❌ 未載入")


def mem_refresh_all():
    """
    重新整理「模型記憶體管理」Tab 的所有狀態顯示。
    同時查詢 Ollama（LLM）和 Model Server（OCR + Embedding）的狀態。
    回傳：(status_msg, running_html, ocr_status, emb_status, model_choices)
    """
    html   = _get_ollama_running_models_html()
    ocr_st = _get_ocr_status()
    emb_st = _get_emb_status()
    models = get_model_list()
    return ("✅ 狀態已更新", html, ocr_st, emb_st, gr.update(choices=models))


def ollama_unload_model(model_name):
    """
    透過 keep_alive=0 讓 5090 的 Ollama 釋放指定模型的 VRAM。
    Ollama 完成後立即卸載模型，不保留快取。
    """
    if not model_name:
        return "❌ 請先選擇要卸載的模型", _get_ollama_running_models_html()
    try:
        _ollama_client.generate(model=model_name, prompt="", keep_alive=0)
        return (f"✅ 已傳送 keep_alive=0，{model_name} 應已從記憶體卸載",
                _get_ollama_running_models_html())
    except Exception as e:
        return f"❌ 卸載失敗：{e}", _get_ollama_running_models_html()


def ollama_load_model(model_name):
    """
    Warm-up：讓 5090 的 Ollama 把指定模型預載入 VRAM，縮短第一次問答的等待時間。
    """
    if not model_name:
        return "❌ 請先選擇要載入的模型", _get_ollama_running_models_html()
    try:
        _ollama_client.generate(model=model_name, prompt="")
        return (f"✅ {model_name} 已 warm-up 完成，模型已載入記憶體",
                _get_ollama_running_models_html())
    except Exception as e:
        return f"❌ 載入失敗：{e}", _get_ollama_running_models_html()


def ocr_unload_model():
    """
    通知 5090 Model Server 卸載 OCR 模型，釋放 GPU VRAM（POST /ocr/unload）。
    5090 端執行：remove_hook → .cpu() → gc.collect() → empty_cache()
    """
    result = _model_server_post("/ocr/unload")
    if "error" in result:
        return f"❌ 卸載失敗：{result['error']}", _get_ocr_status()
    return "✅ OCR 模型已卸載，5090 VRAM 已釋放", _get_ocr_status()


def ocr_reload_model():
    """
    通知 5090 Model Server 重新載入 OCR 模型（POST /ocr/load）。
    載入期間 UI 會顯示「⏳ 正在載入...」（使用 generator）。
    """
    yield "⏳ 通知 5090 載入 OCR 模型，請稍候...", _get_ocr_status()
    result = _model_server_post("/ocr/load")
    if "error" in result:
        yield f"❌ OCR 模型載入失敗：{result['error']}", _get_ocr_status()
    else:
        yield "✅ OCR 模型重新載入完成（5090）", _get_ocr_status()


def emb_unload_model():
    """
    通知 5090 Model Server 卸載 Embedding 模型（POST /embed/unload）。
    同時將本地 RAG_AVAILABLE 設為 False，暫停問答功能。
    """
    global RAG_AVAILABLE
    result = _model_server_post("/embed/unload")
    if "error" in result:
        return f"❌ 卸載失敗：{result['error']}", _get_emb_status()
    RAG_AVAILABLE = False
    return "✅ Embedding 模型已卸載（5090），RAG 問答暫停", _get_emb_status()


def emb_reload_model():
    """
    通知 5090 Model Server 重新載入 Embedding 模型（POST /embed/load）。
    同時恢復 RAG_AVAILABLE = True，讓問答功能重新可用。
    載入期間 UI 會顯示「⏳ 正在載入...」（使用 generator）。
    """
    global RAG_AVAILABLE
    yield "⏳ 通知 5090 載入 Embedding 模型，請稍候...", _get_emb_status()
    result = _model_server_post("/embed/load")
    if "error" in result:
        yield f"❌ Embedding 模型載入失敗：{result['error']}", _get_emb_status()
    else:
        RAG_AVAILABLE = True
        yield "✅ Embedding 模型重新載入完成，RAG 問答功能已恢復", _get_emb_status()
...
def run_ocr_extraction(state_data, progress=gr.Progress()):
    ...
    OCR 模型懶載入：啟動時不載入 OCR（省 6–7 GB VRAM），
    第一次使用時自動觸發 load_ocr_model()，載入完再執行 OCR。
    """
    split_path = state_data.get("split_pdf_path")
    if not split_path:
        yield "❌ 無切割檔", None, state_data, gr.update(visible=True), gr.update(visible=False)
        return

    try:
        # 遠端 OCR：直接呼叫 5090 Model Server API，無需在 Client 載入模型
        yield ("⏳ 傳送至 5090 執行 OCR（首次可能需要 30–60 秒載入模型）...",
               None, state_data, gr.update(visible=False), gr.update(visible=False))

        msg, txt_path = pdf_process_and_save(
            split_path, progress=progress
        )
        state_data["txt_path"] = txt_path
        yield (f"✅ {msg}", txt_path, state_data,
               gr.update(visible=False), gr.update(visible=True))
    except Exception as e:
        yield (f"❌ 失敗: {e}", None, state_data,
               gr.update(visible=True), gr.update(visible=False))
...
def go_to_step4(state_data, progress=gr.Progress()):
    ...
    預設 LLM 模型：下拉選單預設顯示 qwen3.5:4b，若不存在則顯示列表第一個。
    """
    ...
    default_model  = "qwen3.5:4b" if "qwen3.5:4b" in current_models else current_models[0]
...
def smart_search_and_confirm(doc_description, user_question, model, lang,
    ...
    if folder_index is None:
        return (None, "<div style='color:red;padding:20px;'><h3>文件索引建立中</h3></div>",
                *empty[1:], "索引未建立")
...
with gr.Blocks(title="智能文件處理系統") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("🔍 智能檢索回答"):
            ...
            with gr.Row():
                ...
                smart_model_selector = gr.Dropdown(choices=get_model_list(), value="qwen3.5:4b",
                                                   label="🤖 AI 模型", scale=1)
            ...
            with gr.Group(visible=False) as smart_qa_area:
                ...
                with gr.Group(visible=False) as smart_cand_group:
                    ...
                    with gr.Row():
                        ...
                        smart_regen_btn     = gr.Button("🔄 不對，重新用AI生成", variant="secondary", scale=1)
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=3):
                        ...
                        with gr.Row():
                            ...
                            smart_send_btn    = gr.Button("送出問題", variant="primary")
                        with gr.Row():
                            smart_abort_btn     = gr.Button("⛔ 截斷生成", variant="stop", scale=1)
                            save_qa_btn_smart   = gr.Button("💾 儲存此回答案", variant="secondary", scale=1)
                    ...
                    with gr.Column(scale=2):
                        with gr.Group(visible=False) as smart_info_panel:
                            gr.Markdown("### 🔍 檢索原始條款")
            ...
            with gr.Group(visible=False) as step4:
                ...
                with gr.Row():
                    rag_status     = gr.Textbox(label="📡 知識庫狀態", value="尚未建立索引",
                                                interactive=False, scale=2)
                    model_selector = gr.Dropdown(choices=get_model_list(), value="qwen3.5:4b",
                ...
                with gr.Group(visible=False) as step4_cand_group:
                    ...
                    with gr.Row():
                        step4_use_cache_btn = gr.Button("✅ 使用此快取答案", variant="primary",
                                                        visible=False, scale=1)
                        step4_regen_btn  = gr.Button("🔄 不對，重新用AI生成", variant="secondary", scale=1)
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=3):
                        ...
                        with gr.Row():
                            ...
                            send_btn        = gr.Button("送出問題", variant="primary", scale=2)
                        with gr.Row():
                            step4_abort_btn  = gr.Button("⛔ 截斷生成", variant="stop", scale=1)
                            save_qa_btn_4    = gr.Button("💾 儲存此回答案", variant="secondary", scale=1)
                    ...
                    with gr.Column(scale=2):
                        with gr.Group(visible=False) as side_info_panel:
                            gr.Markdown("### 🔍 檢索原始條款")
                            retrieval_info_area = gr.HTML(
                                value="<div padding:20px;'>"
        ...
        with gr.Tab("🧠 模型記憶體管理"):
            gr.Markdown("## 🧠 模型記憶體管理")
            gr.Markdown(
                "管理載入到記憶體的模型。透過 **keep_alive=0** 卸載 Ollama 模型釋放 VRAM，"
                "或重新 warm-up 讓模型快取回記憶體，縮短下次回答的等待時間。"
            )

            with gr.Row():
                mem_refresh_btn = gr.Button("🔄 重新整理狀態", variant="secondary", scale=1)
                mem_status_box  = gr.Textbox(label="📡 系統訊息", interactive=False,
                                             value="點選「重新整理狀態」查看目前狀態", scale=3)

            gr.Markdown("---")

            # ── Ollama LLM 區 ─────────────────────────────────
            gr.Markdown("### 🤖 Ollama LLM（視訊記憶體 VRAM）")
            with gr.Row():
                ollama_model_sel = gr.Dropdown(
                    choices=get_model_list(), label="選擇 Ollama 模型",
                    info="下方操作都會作用於選定的模型", scale=2
                )
                ollama_unload_btn = gr.Button("⏏️ 卸載（keep_alive=0）",
                                              variant="stop", scale=1)
                ollama_load_btn   = gr.Button("▶️ 載入 / Warm-up",
                                              variant="primary", scale=1)
            ollama_op_msg = gr.Textbox(label="Ollama 操作結果", interactive=False)

            # ── Ollama 目前已載入模型列表 ─────────────────────
            gr.Markdown("#### 📋 Ollama 目前佔用記憶體的模型")
            ollama_running_html = gr.HTML(
                value="<p padding:10px;'>點「重新整理狀態」載入</p>"
            )

            gr.Markdown("---")

            # ── OCR 模型區 ────────────────────────────────────
            gr.Markdown("### 🔬 OCR 模型（HuggingFace — GPU VRAM）")
            gr.Markdown(
                "> OCR 模型（OCRFlux-3B）在應用程式啟動時載入。"
                "卸載後可透過「重新載入」按鈕恢復，無需重啟程式。"
                "載入約需 30–60 秒，請耐心等待。"
            )
            with gr.Row():
                ocr_status_display = gr.Textbox(
                    label="OCR 模型狀態", interactive=False,
                    value="點「重新整理狀態」查看", scale=3
                )
                ocr_unload_btn = gr.Button("⏏️ 卸載 OCR 模型", variant="stop", scale=1)
                ocr_reload_btn = gr.Button("▶️ 重新載入 OCR", variant="primary", scale=1)
            ocr_op_msg = gr.Textbox(label="OCR 操作結果", interactive=False)

            # ── Embedding 模型區 ──────────────────────────────
            gr.Markdown("### 📐 Embedding 模型（HuggingFace — GPU VRAM）")
            gr.Markdown(
                "> Yuan-embedding-2.0-zh 在啟動時載入，供 RAG 索引建立與搜尋使用。"
                "卸載後可透過「重新載入」按鈕恢復，無需重啟程式。"
                "載入約需 10–30 秒。"
            )
            with gr.Row():
                emb_status_display = gr.Textbox(
                    label="Embedding 模型狀態", interactive=False,
                    value="點「重新整理狀態」查看", scale=3
                )
                emb_unload_btn = gr.Button("⏏️ 卸載 Embedding 模型", variant="stop", scale=1)
                emb_reload_btn = gr.Button("▶️ 重新載入 Embedding", variant="primary", scale=1)
            emb_op_msg = gr.Textbox(label="Embedding 操作結果", interactive=False)

        # ══════════════════════════════════════════════════════
        # Tab 6: 使用者說明
        # ══════════════════════════════════════════════════════
        with gr.Tab("🗑️ 刪除資料夾"):
            gr.Markdown("## 🗑️ 刪除合約資料夾")
            gr.Markdown(
                "選擇要刪除的資料夾後點擊「確認刪除」。\n\n"
                "⚠️ **刪除後無法復原**，將移除該資料夾內所有圖片、文字、Embedding 快取與問答快取。"
            )
            with gr.Row():
                del_folder_dropdown = gr.Dropdown(
                    label="選擇要刪除的資料夾",
                    choices=get_history_folders(),
                    value=None,
                    scale=4,
                )
                del_refresh_btn = gr.Button("🔄 重新整理", scale=1)
            with gr.Row():
                del_preview_btn = gr.Button("👁️ 預覽資料夾內容", variant="secondary", scale=2)
                del_confirm_btn = gr.Button("🗑️ 確認刪除", variant="stop", scale=1)
            del_preview_gallery = gr.Gallery(
                label="資料夾預覽",
                columns=4, height=260,
                visible=False,
            )
            del_preview_info  = gr.Textbox(label="資料夾資訊", interactive=False, visible=False)
            del_status_msg    = gr.Textbox(label="操作結果", interactive=False)

        with gr.Tab("⚡ 快速問答（不存檔）"):
            gr.Markdown("## ⚡ 快速問答模式")
            gr.Markdown(
                "流程與「檔案上傳」相同（上傳 → 預覽切割 → OCR → 問答），"
                "但**不寫入共用資料夾、不留任何快取**。\n\n"
                "適合臨時查看不需要儲存的文件。"
            )

            # 內部 State（全部獨立，不與 Tab 2 共用）
            temp_file_state      = gr.State({"original_file": None, "file_ext": None,
                                             "converted_pdf": None, "split_pdf_path": None,
                                             "txt_path": None})
            temp_engine_state    = gr.State(None)
            temp_txt_path_state  = gr.State(None)
            temp_tmpdir_state    = gr.State(None)
            temp_info_visible    = gr.State(False)
            temp_img_visible     = gr.State(False)
            temp_pending_qa      = gr.State(None)
            temp_pending_q       = gr.State("")

            # ── Step 1：上傳 ──────────────────────────────────
            with gr.Group(visible=True) as temp_step1:
                gr.Markdown("## 1️⃣ 上傳檔案")
                temp_file_input   = gr.File(label="PDF / DOCX",
                                            file_types=[".pdf", ".docx"], type="filepath")
                temp_upload_msg   = gr.Textbox(interactive=False, show_label=False)
                temp_next_btn     = gr.Button("▶️ 下一步：轉換與預覽", variant="primary")

            # ── Step 2：預覽與切割 ────────────────────────────
            with gr.Group(visible=False) as temp_step2:
                gr.Markdown("## 2️⃣ 預覽與切割")
                temp_preview_gallery = gr.Gallery(label="預覽", columns=5,
                                                  height=250, object_fit="contain")
                temp_page_hint    = gr.Markdown("頁數資訊")
                with gr.Row():
                    temp_page_start = gr.Number(label="起始頁數", value=1,
                                                precision=0, minimum=1, step=1)
                    temp_page_end   = gr.Number(label="結束頁數", value=1,
                                                precision=0, minimum=1, step=1)
                with gr.Row():
                    temp_back_2_1 = gr.Button("◀️ 返回", variant="secondary")
                    temp_split_btn = gr.Button("🔪 執行切割", variant="primary")

            # ── Step 3：OCR ───────────────────────────────────
            with gr.Group(visible=False) as temp_step3:
                gr.Markdown("## 3️⃣ 切割結果與 OCR")
                temp_result_msg    = gr.Textbox(label="系統訊息", interactive=False)
                with gr.Row():
                    temp_result_file    = gr.File(label="切割後的 PDF")
                    temp_ocr_result_file = gr.File(label="文字提取結果 (.txt)")
                with gr.Row():
                    temp_back_3_2  = gr.Button("◀️ 返回切割", variant="secondary")
                    temp_ocr_btn   = gr.Button("🔍 執行 OCR / 文字提取", variant="primary")
                    temp_to_rag_btn = gr.Button("▶️ 下一步：智能問答 (RAG)",
                                                variant="primary", visible=False)

            # ── Step 4：問答 ──────────────────────────────────
            with gr.Group(visible=False) as temp_step4:
                gr.Markdown("## 4️⃣ 智能問答 (RAG)　⚠️ 本模式不儲存問答快取")
                with gr.Row():
                    temp_rag_status  = gr.Textbox(label="📡 知識庫狀態",
                                                  value="尚未建立索引",
                                                  interactive=False, scale=2)
                    temp_model_sel   = gr.Dropdown(choices=get_model_list(),
                                                   value="qwen3.5:4b",
                                                   label="🤖 選擇 AI 模型", scale=1)
                    temp_lang_input  = gr.Textbox(label="🌐 輸出語言",
                                                  value="中文", scale=1)

                with gr.Row():
                    temp_show_img_btn = gr.Button("📸 顯示/隱藏文件圖片",
                                                  variant="secondary", scale=0)

                with gr.Group(visible=False) as temp_img_group:
                    temp_img_gallery = gr.Gallery(label="文件原稿圖片", columns=4,
                                                  height=350, object_fit="contain")

                with gr.Row(equal_height=True):
                    with gr.Column(scale=3):
                        temp_chatbot  = gr.Chatbot(type="messages",
                                                   label="智能助手", height=550,
                                                   show_label=False)
                        temp_msg      = gr.Textbox(placeholder="請輸入關於此合約的問題...",
                                                   show_label=False, container=False)
                        with gr.Row(variant="panel"):
                            temp_use_full = gr.Checkbox(label="包含完整章節(生成時間較長)",
                                                        value=True, interactive=True)
                        with gr.Row():
                            temp_back_4_3    = gr.Button("◀️ 返回上一步",
                                                         variant="secondary", scale=1)
                            temp_info_toggle = gr.Button("👁️ 顯示/隱藏 檢索相關條款",
                                                         variant="secondary", scale=1)
                            temp_send        = gr.Button("送出問題",
                                                         variant="primary", scale=2)
                        with gr.Row():
                            temp_abort_btn = gr.Button("⛔ 截斷生成",
                                                       variant="stop", scale=1)
                            temp_regen     = gr.Button("🔄 重新用AI生成",
                                                       variant="secondary", scale=1)
                            temp_clear     = gr.Button("🗑️ 清除對話",
                                                       variant="secondary", scale=1)

                    with gr.Column(scale=2):
                        with gr.Group(visible=False) as temp_side_info:
                            gr.Markdown("### 🔍 檢索原始條款")
                            temp_retrieval_info = gr.HTML(
                                value="<div style='padding:20px;'>"
                                      "詢問問題後，相關的合約原文將顯示於此...</div>")

                gr.Markdown("---")
                temp_reset_btn = gr.Button("🔄 重新開始", variant="secondary")
    ...
    def _auto_step4(message, txt_path, history, rag_engine,
                    use_full_context, model, lang):
        """
        Step 4 送出問題：自動判斷快取 / LLM。
        相似度 >= 0.8 → 直接套用快取答案；否則 → 呼叫 LLM。
        """
        return auto_qa_response(
            message, txt_path, history, rag_engine,
            use_full_context, model, lang, rag_available=RAG_AVAILABLE
        )

    def _auto_smart(message, txt_path, history, rag_engine,
                    use_context, model, lang):
        """
        智能搜尋 Tab 送出問題：邏輯與 _auto_step4 相同。
        """
        return auto_qa_response(
            message, txt_path, history, rag_engine,
            use_context, model, lang, rag_available=RAG_AVAILABLE
        )

    # 送出問題：全自動快取比對 / LLM 生成
    # 相似度 >= 0.8 → 直接投放快取答案；< 0.8 → 呼叫 LLM
    # 完成後用 .then 更新 step4_pending_q（供「呼叫AI 生成」覆蓋按鈕使用）
    for trigger in [msg.submit, send_btn.click]:
        trigger(_auto_step4,
                [msg, step4_txt_path_state, chatbot, rag_engine_state,
                 use_full_context_chk, model_selector, lang_input],
                [chatbot, retrieval_info_area, side_info_panel,
                 step4_pending_qa, step4_cand_group]
                ).then(lambda m: m, [msg], [step4_pending_q]
    ...
    # ── 智能檢索 Tab ───────────────────────────────────────────
    # 應用程式啟動時自動建立資料夾索引
    def _build_index_with_status():
        print("[load] 開始建立資料夾索引...")
        idx = build_folder_metadata_index()[0]
        print("[load] 索引建立完成")
        return idx
    demo.load(_build_index_with_status, [], [folder_index_state], queue=False)
    ...
    smart_search_btn.click(smart_search_and_confirm,
        ...
        [smart_chatbot, confirm_display, confirm_yes_btn, confirm_no_btn,
         ...
         search_results_state, smart_retrieval_info],
    ...
    lambda: (gr.update(visible=False), gr.update(value=[]), False, False),
    [],
    [smart_img_group, smart_img_gallery, smart_img_visible, smart_info_visible]
    ).then(lambda: gr.update(visible=False), [], [smart_info_panel]
    ...
    [smart_img_group, smart_img_gallery, smart_img_visible, smart_info_visible]
        ...
        [smart_user_question, smart_txt_path_state, smart_chatbot, smart_rag_engine,
         smart_use_context_chk, smart_model_selector, smart_lang_input],
        [smart_chatbot, smart_retrieval_info, smart_info_panel,
         smart_pending_qa, smart_cand_group]
    ).then(lambda q: q, [smart_user_question], [smart_pending_q])
    ...
    # 智能搜尋 Tab：同樣全自動快取 / LLM
    for trigger in [smart_followup_msg.submit, smart_send_btn.click]:
        trigger(_auto_smart,
                [smart_followup_msg, smart_txt_path_state,
                 smart_chatbot, smart_rag_engine,
                 smart_use_context_chk, smart_model_selector, smart_lang_input],
                [smart_chatbot, smart_retrieval_info, smart_info_panel,
                 smart_pending_qa, smart_cand_group]
                ).then(lambda m: m, [smart_followup_msg], [smart_pending_q]
    ...
    # ── 模型記憶體管理 Tab ─────────────────────────────────────
    _mem_refresh_outputs = [mem_status_box, ollama_running_html,
                            ocr_status_display, emb_status_display, ollama_model_sel]

    mem_refresh_btn.click(mem_refresh_all, [], _mem_refresh_outputs)

    ollama_unload_btn.click(ollama_unload_model, [ollama_model_sel],
                            [ollama_op_msg, ollama_running_html])

    ollama_load_btn.click(ollama_load_model, [ollama_model_sel],
                          [ollama_op_msg, ollama_running_html])

    ocr_unload_btn.click(ocr_unload_model, [],
                         [ocr_op_msg, ocr_status_display])

    ocr_reload_btn.click(ocr_reload_model, [],
                         [ocr_op_msg, ocr_status_display])

    emb_unload_btn.click(emb_unload_model, [],
                         [emb_op_msg, emb_status_display])

    emb_reload_btn.click(emb_reload_model, [],
                         [emb_op_msg, emb_status_display])
    ...
    # 頁面載入時自動刷新模型記憶體狀態
    demo.load(mem_refresh_all, [], _mem_refresh_outputs,queue=False)
    ...
    def _qa_refresh(folder):
        ...
        folders = get_history_folders()
        html = render_qa_html(folder)
        choices = get_qa_entry_choices(folder)
        return (gr.update(choices=folders, value=folder if folder in folders else None),
                html, choices)
    ...
    qa_folder_dropdown.change(
        ...
        outputs=[qa_entries_html, qa_entry_dropdown],
    )
    ...
    qa_refresh_btn.click(
        _qa_refresh,
        inputs=[qa_folder_dropdown],
        outputs=[qa_folder_dropdown, qa_entries_html, qa_entry_dropdown],
        queue=False,
    )
    ...
    del_refresh_btn.click(
        lambda: gr.update(choices=get_history_folders(), value=None),
        outputs=[del_folder_dropdown],
        queue=False,
    )

    def _del_preview(folder_name):
        imgs, info = preview_history_folder(folder_name)
        return (
            gr.update(value=imgs, visible=bool(imgs)),
            gr.update(value=info, visible=True),
        )

    del_preview_btn.click(
        _del_preview,
        inputs=[del_folder_dropdown],
        outputs=[del_preview_gallery, del_preview_info],
    )

    def _del_confirm(folder_name):
        status, choices, dd_update = delete_folder(folder_name)
        return (
            status,
            dd_update,
    ...
    del_confirm_btn.click(
        _del_confirm,
        inputs=[del_folder_dropdown],
        outputs=[del_status_msg, del_folder_dropdown,
                 del_preview_gallery, del_preview_info],
    )

    # ── 快速問答（不存檔）Tab 事件綁定 ────────────────────────

    def _temp_process_upload(file, state_data):
        """Step 1 → 2：上傳檔案，偵測格式、轉換 DOCX、產生預覽。不偵測同名資料夾。"""
        ...
        # 每次上傳新檔時清理上次的 tempdir
        prev_tmpdir = state_data.get("tmpdir")
        if prev_tmpdir and os.path.exists(prev_tmpdir):
            import shutil as _sh
            try: _sh.rmtree(prev_tmpdir)
            except Exception: pass

        # 建立本次專屬的 tempdir（取代 TEMP_DIR，不影響公共暫存）
        tmpdir = _tf.mkdtemp(prefix="sinbon_temp_upload_")
        ...
        try:
            if ext == ".pdf":
                ...
                pdf_path = str(docx_to_pdf_libreoffice(str(file_path), Path(tmpdir)))
                new_state["converted_pdf"] = pdf_path
            else:
                return ["❌ 格式錯誤", *fail[1:]]
    ...
    temp_next_btn.click(
        _temp_process_upload,
        [temp_file_input, temp_file_state],
        [temp_upload_msg, temp_file_state, temp_preview_gallery, temp_page_hint,
         temp_page_start, temp_page_end,
         temp_step1, temp_step2, temp_step3, temp_step4, temp_result_msg],
    )

    def _temp_split_document(state_data, start_page, end_page):
        """Step 2 → 3：切割 PDF，輸出到 state_data["tmpdir"]，不用 TEMP_DIR。"""
        fail = [
            "", None, state_data,
            gr.update(visible=True), gr.update(visible=False),
            gr.update(visible=False), gr.update(visible=False), "", None,
        ...
        tmpdir = state_data.get("tmpdir", str(TEMP_DIR))
        ...
        try:
            ...
            # 切割到 tmpdir（不寫入公共 TEMP_DIR）
            out_path = split_pdf(target, start, end, Path(state_data["original_file"]).stem)
            # split_pdf 預設寫到 TEMP_DIR；把結果複製到 tmpdir
            import shutil as _sh
            dest = Path(tmpdir) / out_path.name
            _sh.copy2(str(out_path), str(dest))
            state_data["split_pdf_path"] = str(dest)

            return [
                f"✅ 切割完成: {dest.name}", str(dest), state_data,
                gr.update(visible=False), gr.update(visible=True), gr.update(visible=True),
                "", None, gr.update(visible=False),
            ]
        except Exception as e:
    ...
    temp_split_btn.click(
        _temp_split_document,
        [temp_file_state, temp_page_start, temp_page_end],
        [temp_result_msg, temp_result_file, temp_file_state,
         temp_step2, temp_step3,
         temp_ocr_btn, temp_result_msg, temp_ocr_result_file, temp_to_rag_btn],
    )

    def _temp_run_ocr(state_data, progress=gr.Progress()):
        """Step 3 OCR：呼叫 pdf_process_temp，結果存在 tmpdir 不寫公共目錄。"""
        split_path = state_data.get("split_pdf_path")
        if not split_path:
            yield "❌ 無切割檔", None, state_data, gr.update(visible=True), gr.update(visible=False)
            return
        try:
            # 遠端 OCR：直接呼叫 5090 Model Server API
            yield ("⏳ 傳送至 5090 執行 OCR...",
                   None, state_data, gr.update(visible=False), gr.update(visible=False))

            msg, txt_path, _ = pdf_process_temp(
                split_path, progress=progress
            )
            state_data["txt_path"] = txt_path
            yield (f"✅ {msg}", txt_path, state_data,
    ...
    temp_ocr_btn.click(
        _temp_run_ocr, [temp_file_state],
        [temp_result_msg, temp_ocr_result_file, temp_file_state,
         temp_ocr_btn, temp_to_rag_btn],
        queue=True,
    )

    def _temp_go_to_step4(state_data, progress=gr.Progress()):
        """Step 3 → 4：用 build_temp_rag_index 建立純記憶體索引，不寫快取。"""
        txt_path = state_data.get("txt_path")
        if not txt_path:
            yield (gr.update(visible=True), gr.update(visible=False),
                   gr.update(value=[]), "❌ 找不到文字檔", None, gr.update(), None)
        ...
        yield (gr.update(visible=False), gr.update(visible=True),
               gr.update(value=[]), "⏳ 正在建立知識庫（不存入快取）...", None,
               gr.update(choices=current_models, value=default_model), None)

        try:
            engine, status = build_temp_rag_index(txt_path, RAG_AVAILABLE, progress=progress)
            yield (gr.update(visible=False), gr.update(visible=True),
                   gr.update(value=[]), status, engine,
                   gr.update(choices=current_models), txt_path)
        except Exception as e:
    ...
    temp_to_rag_btn.click(
        _temp_go_to_step4, [temp_file_state],
        [temp_step3, temp_step4, temp_chatbot, temp_rag_status,
         temp_engine_state, temp_model_sel, temp_txt_path_state],
    )

    # ── Step 4 工具 ───────────────────────────────────────────
    temp_show_img_btn.click(
        toggle_doc_images,
        [temp_txt_path_state, temp_img_visible],
        [temp_img_group, temp_img_gallery, temp_img_visible],
    )

    temp_info_toggle.click(
        lambda v: (gr.update(visible=not v), not v),
        [temp_info_visible], [temp_side_info, temp_info_visible],
    )
    ...
    temp_back_2_1.click(
        lambda: [gr.update(visible=True), gr.update(visible=False),
                 gr.update(visible=False), gr.update(visible=False)],
        [], [temp_step1, temp_step2, temp_step3, temp_step4],
    )
    temp_back_3_2.click(
    ...
    def _temp_reset(state_data):
        """重設 temp tab 到初始狀態，清除所有 State，安全刪除 tmpdir。"""
        import shutil as _sh
        # 安全清理 tmpdir：只在路徑合法且為 tempdir 前綴時才刪除
        if state_data:
            tmpdir = state_data.get("tmpdir")
            if tmpdir and os.path.isdir(tmpdir) and "sinbon_temp_" in tmpdir:
                try:
                    _sh.rmtree(tmpdir, ignore_errors=True)
                except Exception:
                    pass

        empty = {"original_file": None, "file_ext": None,
                 "converted_pdf": None, "split_pdf_path": None,
                 "txt_path": None, "tmpdir": None}
        return [
            gr.update(value=None, interactive=True), empty, "",
            1, 1,
            gr.update(visible=True),  gr.update(visible=False),
            gr.update(visible=False), gr.update(visible=False),
            None, "",
            ...
            "尚未建立索引", None, [], "",
            gr.update(visible=False), "中文", True,
            None, False,
        ]

    temp_reset_btn.click(
        _temp_reset, [temp_file_state],
        [temp_file_input, temp_file_state, temp_upload_msg,
         temp_page_start, temp_page_end,
         temp_step1, temp_step2, temp_step3, temp_step4,
         temp_preview_gallery, temp_page_hint,
         temp_result_msg, temp_result_file, temp_ocr_result_file, temp_ocr_btn,
         temp_rag_status, temp_engine_state, temp_chatbot, temp_retrieval_info,
         temp_side_info, temp_lang_input, temp_use_full,
         temp_txt_path_state, temp_img_visible],
    )

    # ── Step 4 問答 ───────────────────────────────────────────
    def _temp_auto_qa(message, txt_path, history, engine, use_full, model, lang):
        return auto_qa_response(
            message, txt_path, history, engine,
            use_full, model, lang, rag_available=RAG_AVAILABLE
        )

    for trigger in [temp_msg.submit, temp_send.click]:
        trigger(
            _temp_auto_qa,
            [temp_msg, temp_txt_path_state, temp_chatbot,
             temp_engine_state, temp_use_full, temp_model_sel, temp_lang_input],
            [temp_chatbot, temp_retrieval_info, temp_side_info,
             temp_pending_qa, gr.State(None)],
        ).then(lambda m: m, [temp_msg], [temp_pending_q]
        ).then(lambda: "", None, temp_msg)

    temp_regen.click(
        rag_chat_response,
        [temp_pending_q, temp_chatbot, temp_engine_state,
         temp_model_sel, temp_lang_input, temp_use_full, temp_txt_path_state],
        [temp_chatbot, temp_retrieval_info, temp_side_info,
         temp_pending_qa, gr.State(None)],
    )

    temp_abort_btn.click(request_abort, [], [])
    temp_clear.click(lambda: [], None, temp_chatbot, queue=False)

# ── 啟動 ───────────────────────────────────────────────────────
# server_name="0.0.0.0"：允許區域網路內其他裝置連線（如用平板操作）
# server_port=8080：預設埠號，衝突時可改為其他埠號
demo.queue()

...
【六個 Tab】
  ...
  Tab 5：🧠 模型記憶體管理 — 卸載 / 重新載入 OCR / Embedding / Ollama 模型
  Tab 6：🗑️ 刪除資料夾 — 完整刪除指定合約資料夾（含快取）
  Tab 7：⚡ 快速問答（不存檔）— 上傳 PDF 直接 OCR + RAG，不寫入共用目錄
  Tab 8：📖 使用者說明
...
def load_ocr_model():
    """
    載入（或重新載入）OCRFlux-3B 模型到 GPU。
    修改全域變數 ocr_model / ocr_tokenizer / ocr_processor。

    【可重複呼叫】
    卸載後可再次呼叫此函式重新載入，不需重啟程式。
    torch_dtype="auto"：自動選精度（通常 float16），節省 VRAM。
    device_map="auto"：自動分配到 GPU；無 GPU 則退到 CPU（極慢）。

    Returns
    -------
    bool : 載入成功為 True，失敗為 False
    """
...
def load_embedding_model():
    """
    載入（或重新載入）Yuan-embedding-2.0-zh Embedding 模型。
    同時初始化 llama_index Settings.llm（Ollama）與 Settings.embed_model。
    修改全域變數 RAG_AVAILABLE。

    【可重複呼叫】
    卸載後可再次呼叫此函式重新載入，不需重啟程式。
    device="cpu"：Embedding 計算使用 CPU，節省 VRAM 給 Ollama LLM 使用。
    Embedding 是離線批次計算，放 CPU 速度差異不明顯但可省下 1–2 GB VRAM。
    ...
    try:
        ...
        # ── Ollama LLM client（只是設定，不會實際載入模型進 VRAM）────
        # Ollama server 是獨立背景程序，keep_alive 預設 5 分鐘。
        # 若上次使用後 server 未重啟，qwen3:4b 可能還在 VRAM。
        # 這裡主動送 keep_alive=0 強制卸載，讓 VRAM 保持乾淨，
        # 只有使用者真正送問題時才讓 Ollama 把模型載入。
        try:
            import ollama as _ollama
            _ollama.generate(model="qwen3:4b", prompt="", keep_alive=0)
            print("✅ Ollama qwen3:4b 已從 VRAM 卸載（等待使用時再自動載入）")
        except Exception:
            pass   # Ollama server 未啟動或模型不存在，靜默忽略
...
# ── 程式啟動時只初始化 Embedding / LLM，不載入 OCR ──────────────
# OCR 模型（OCRFlux-3B）約佔 6–7 GB VRAM，但只有使用者執行 OCR 時才需要。
# 改為懶載入：使用者點「執行 OCR」時自動觸發 load_ocr_model()。
# 這樣系統待機時可省下 6–7 GB VRAM 給 Ollama LLM 使用。
...
def _get_ollama_running_models_html():
    """
    查詢 Ollama 目前佔用記憶體（VRAM/RAM）的模型，
    回傳 HTML 表格字串供 UI 顯示。
    使用 ollama.ps() 取得執行中的模型列表。
    """
    try:
        result = ollama.ps()
...
def _get_ocr_status():
    ...
    if ocr_model is None:
        return "❌ 未載入（已卸載或初始化失敗）"
    try:
        import torch
        dev = next(ocr_model.parameters()).device
        return f"✅ 已載入（device: {dev}）"
    except Exception:
        return "✅ 已載入"


def _get_emb_status():
    """回傳 Embedding 模型目前的載入狀態文字。"""
    try:
        from llama_index.core import Settings
        if Settings.embed_model is None:
            return "❌ 未載入"
        return f"✅ 已載入（{Settings.embed_model.model_name}）"
    except Exception:
        return "❌ 無法取得狀態"
...
def ollama_unload_model(model_name):
    """
    透過 keep_alive=0 讓 Ollama 立即釋放指定模型的記憶體。
    原理：向 Ollama 送出一個空 prompt，並設定 keep_alive=0，
    Ollama 完成後立即將模型從記憶體卸載，而不保留快取。
    """
    if not model_name:
...
def ollama_load_model(model_name):
    """
    Warm-up：送出空 prompt（keep_alive 使用 Ollama 預設值 5 分鐘），
    讓 Ollama 把模型載入到記憶體並保持快取，縮短下次回答的等待時間。
    """
    if not model_name:
...
def ocr_unload_model():
    """
    卸載 OCRFlux-3B 模型，釋放 GPU VRAM。

    device_map="auto" 載入的模型，accelerate 會在每層附加 dispatch hook，
    這些 hook 持有 CUDA tensor 引用，導致光 gc.collect() + empty_cache() 不夠。
    正確步驟：
      1. remove_hook_from_module：移除 accelerate dispatch hook
      2. ocr_model.cpu()：強制所有 tensor 離開 GPU
      3. = None：釋放 Python 引用
      4. gc.collect()：讓 GC 回收物件
      5. empty_cache()：把 PyTorch 快取還給 OS
    """
    global ocr_model, ocr_tokenizer, ocr_processor
    if ocr_model is None:
        return "⚠️ OCR 模型已是未載入狀態", _get_ocr_status()
    try:
        import torch, gc
        # 移除 accelerate 的 dispatch hook（device_map="auto" 時附加）
        try:
            from accelerate.hooks import remove_hook_from_module
            remove_hook_from_module(ocr_model, recurse=True)
        except Exception:
            pass  # accelerate 未安裝或無 hook，靜默忽略
        # 強制所有 tensor 移回 CPU，確保 CUDA 記憶體立即釋放
        ocr_model.cpu()
        ocr_model = ocr_tokenizer = ocr_processor = None
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        return "✅ OCR 模型已卸載，VRAM 已釋放", _get_ocr_status()
    except Exception as e:
        return f"❌ 卸載失敗：{e}", _get_ocr_status()


def ocr_reload_model():
    """
    重新載入 OCRFlux-3B 模型（呼叫頂層的 load_ocr_model()）。
    可在 UI 卸載後透過按鈕重新載入，不需重啟程式。
    載入期間 UI 會顯示「⏳ 正在載入...」（使用 generator）。
    """
    yield "⏳ 正在載入 OCR 模型，請稍候...", _get_ocr_status()
    ok = load_ocr_model()
    if ok:
        yield "✅ OCR 模型重新載入完成", _get_ocr_status()
    else:
        yield "❌ OCR 模型載入失敗，請檢查路徑或 GPU 記憶體是否足夠", _get_ocr_status()


def emb_unload_model():
    """
    卸載 Yuan-embedding-2.0-zh Embedding 模型，釋放 GPU VRAM。
    同 ocr_unload_model，必須 gc.collect() → empty_cache() 才能
    真正把 VRAM 還給 OS，僅 empty_cache 在 nvidia-smi 上不會有變化。
    """
    global RAG_AVAILABLE
    try:
        import torch, gc
        from llama_index.core import Settings
        if Settings.embed_model is None:
            return "⚠️ Embedding 模型已是未載入狀態", _get_emb_status()
        Settings.embed_model = None
        RAG_AVAILABLE = False
        gc.collect()                          # 先讓 Python GC 回收張量物件引用
        if torch.cuda.is_available():
            torch.cuda.empty_cache()          # 再把 PyTorch 快取還給 OS
        return "✅ Embedding 模型已卸載，VRAM 已釋放", _get_emb_status()
    except Exception as e:
        return f"❌ 卸載失敗：{e}", _get_emb_status()


def emb_reload_model():
    """
    重新載入 Yuan-embedding-2.0-zh Embedding 模型（呼叫頂層的 load_embedding_model()）。
    同時恢復 RAG_AVAILABLE = True，讓問答功能重新可用。
    載入期間 UI 會顯示「⏳ 正在載入...」（使用 generator）。
    """
    yield "⏳ 正在載入 Embedding 模型，請稍候...", _get_emb_status()
    ok = load_embedding_model()
    if ok:
        yield "✅ Embedding 模型重新載入完成，RAG 問答功能已恢復", _get_emb_status()
    else:
        yield "❌ Embedding 模型載入失敗，請檢查 llama_index 安裝或 GPU 記憶體", _get_emb_status()
...
def run_ocr_extraction(state_data, progress=gr.Progress()):
    ...
    try:
        # OCR 懶載入：若模型尚未載入（啟動時跳過 or 曾被卸載），自動載入
        if ocr_model is None:
            yield ("⏳ 首次使用 OCR，正在載入模型（約 30–60 秒）...",
                   None, state_data, gr.update(visible=False), gr.update(visible=False))
            ok = load_ocr_model()
            if not ok:
                yield ("❌ OCR 模型載入失敗，請檢查路徑或 GPU 記憶體是否足夠",
                       None, state_data, gr.update(visible=True), gr.update(visible=False))
                return
        ...
        with gr.Tab("⚡ 快速問答（不存檔）"):
            ...
            with gr.Group(visible=False) as temp_step4:
                ...
                with gr.Row(equal_height=True):
                    ...
                    with gr.Column(scale=2, visible=False) as temp_side_info:
                        gr.Markdown("### 🔍 檢索原始條款")

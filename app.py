...
def ocr_unload_model():
    ...
    使用 global 修改 ocr_model / ocr_tokenizer / ocr_processor，
    並呼叫 torch.cuda.empty_cache() 讓 VRAM 立即回收。
    卸載後可透過 UI「重新載入」按鈕呼叫 ocr_reload_model() 恢復。
    """
    global ocr_model, ocr_tokenizer, ocr_processor
    if ocr_model is None:
    ...
    try:
        import torch
        del ocr_model, ocr_tokenizer, ocr_processor
        ocr_model = ocr_tokenizer = ocr_processor = None
...
def emb_unload_model():
    ...
    透過 llama_index.core.Settings.embed_model = None 釋放引用，
    再呼叫 torch.cuda.empty_cache() 清理殘留的 VRAM。
    卸載後可透過 UI「重新載入」按鈕呼叫 emb_reload_model() 恢復。
    """
    global RAG_AVAILABLE
    try:
        import torch

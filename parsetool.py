...
# BM25 稀疏檢索（選配）
# 安裝：pip install llama-index-retrievers-bm25 jieba
# 未安裝時自動 fallback 為純向量檢索，不影響系統啟動
try:
    ...
    def _jieba_tokenizer(text: str) -> list[str]:
        """
        以 jieba 對中文文字做精確模式分詞，供 BM25Retriever 使用。
        預設的 SimpleTokenizer 以空白切詞，對中文幾乎無效。
        jieba.lcut 回傳 list[str]，符合 BM25Retriever tokenizer 介面。
        """
    ...
    print(f"⚠️  BM25 檢索停用（{_e}）。\n"
          "   安裝指令：pip install llama-index-retrievers-bm25 jieba")
...
class HierarchicalQueryEngine:
    ...
    bm25_retriever : BM25 稀疏關鍵詞檢索器（similarity_top_k=3；套件未安裝時為 None）
    ...
    def __init__(self, index: VectorStoreIndex, nodes: List[TextNode]):
        ...
        self.index          = index
        self.nodes          = nodes
        # similarity_top_k=6：多取三個作為緩衝，
        # 查詢時過濾掉 complete_section 後保留最多 3 個有效節點
        self.base_retriever = index.as_retriever(similarity_top_k=6)
        self._section_index = self._build_section_index()

        # BM25 稀疏檢索器（top-k=3）
        # 只使用 main_clause / sub_clause / header 節點（不含 complete_section）
        # complete_section 是組合節點，加入 BM25 會造成重複且文字過長影響評分
        if _BM25_AVAILABLE:
            retrieval_nodes = [
                n for n in nodes
                if n.metadata.get('clause_type') != 'complete_section'
            ]
            ...
            print(f"✅ BM25 Retriever 建立完成（{len(retrieval_nodes)} 個節點）")
        else:
    ...
    def query_with_complete_sections(self, query: str,
        ...
        【步驟說明】
        Step 1 - 混合檢索：
          向量檢索（base_retriever, top-3）找語意相似節點
          BM25 檢索（bm25_retriever, top-3）找關鍵詞匹配節點
          依 node_id 去重後取最多 6 個節點，向量結果優先
        ...
        # ── 步驟 1：混合檢索（向量 top-3 + BM25 top-3，去重後取最多 6 個）────
        # 向量檢索：語意相似度（embedding cosine）
        # top_k=6：多取三個，過濾掉 complete_section 後保留最多 3 個有效節點
        # complete_section 是組合節點，不應佔用檢索名額（BM25 pool 也排除它們）
        _vector_raw: List[NodeWithScore] = self.base_retriever.retrieve(query)
        vector_nodes: List[NodeWithScore] = [
            n for n in _vector_raw
            if n.node.metadata.get('clause_type') != 'complete_section'
        ][:3]

        # BM25 檢索：關鍵詞稀疏匹配（需安裝 llama-index-retrievers-bm25）
        bm25_nodes: List[NodeWithScore] = (
            self.bm25_retriever.retrieve(query)
            if self.bm25_retriever is not None else []
        )

        # 去重：以 node_id 為 key，向量結果優先（分數保留向量分數）
        # 若同一節點同時出現在兩個結果中，保留向量版本（分數語意較明確）
        seen_ids: dict = {}
        for nws in vector_nodes:
            seen_ids[nws.node.node_id] = nws
        for nws in bm25_nodes:
            if nws.node.node_id not in seen_ids:
                seen_ids[nws.node.node_id] = nws

        retrieved_nodes: List[NodeWithScore] = list(seen_ids.values())

        t1 = time.time()
        print(f"⏱️ [Step 1] 混合檢索耗時: {t1 - st:.4f} 秒 | "
              f"向量={len(vector_nodes)} BM25={len(bm25_nodes)} "
              f"去重後={len(retrieved_nodes)} 個片段")
